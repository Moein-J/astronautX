...
export function HeroContent(props: THeroContentProps) {
  ...
  return (
    <div
      ref={heroContentRef}
      className="relative z-20 my-auto flex w-full max-w-4xl flex-col items-center space-y-3 text-center sm:space-y-5 md:space-y-6"
    >
      ...
      <h1 className="bg-linear-to-r from-white via-cyan-100 to-indigo-300 bg-clip-text px-1 text-2xl leading-[1.15] font-extrabold tracking-tight text-transparent drop-shadow-md sm:text-5xl md:text-7xl">
        {title}
      </h1>

      <p className="max-w-2xl px-2 text-xs leading-relaxed font-normal text-slate-300 sm:px-4 sm:text-base md:text-xl">
        {subtitle}
      </p>

      <div className="flex w-full flex-col items-center justify-center gap-2.5 px-4 pt-1 sm:w-auto sm:flex-row sm:gap-4 sm:px-0 sm:pt-2">
        <a
          href="#journey"
          className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-linear-to-r from-cyan-500 via-indigo-500 to-blue-600 px-6 py-2.5 text-xs font-semibold tracking-wide text-white shadow-lg shadow-cyan-500/30 transition-all duration-300 hover:scale-105 hover:shadow-cyan-500/50 active:scale-95 sm:w-auto sm:px-8 sm:py-3.5 sm:text-base"
        >
          <span>{ctaText}</span>
